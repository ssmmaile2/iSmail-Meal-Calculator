...
const [blockingError, setBlockingError] = useState<BlockingError | null>(null);
  ...
  const mealRisk = useMemo<MealRiskAnalysis>(() => {
    ...
    const redAlert =
      hasHighSodium || (hasHighPotassium && hasHighPhosphorus && hasDenseProtein);
    ...
    if (hasHighPotassium && hasHighPhosphorus && hasDenseProtein) {
      messages.push("الوجبة تجمع بين عنصر مرتفع البوتاسيوم وعنصر مرتفع الفوسفور وبروتين مركز.");
    } else {
    ...
    if (!redAlert && totalSharedLoad >= 4) {
      messages.push("الحمولة المشتركة للوجبة مرتفعة نسبيًا.");
    }
  ...
  const violationsByItem = useMemo(() => {
    ...
    function addGroupConflict(names: string[], builder: (others: string[]) => string) {
      const groupItems = itemsWithFood.filter((item) =>
        names.map(normalizeArabic).includes(normalizeArabic(item.foods.name_ar))
      );

      if (groupItems.length > 1) {
        for (const item of groupItems) {
          const others = groupItems
            .filter((x) => x.id !== item.id)
            .map((x) => x.foods.name_ar);

          if (others.length > 0) addViolation(item.id, builder(others));
        }
      }
    }
  ...
  async function updateQty(itemId: string, newQty: number) {
    ...
    try {
      ...
      setItems((prev) =>
        prev.map((item) => (item.id === itemId ? { ...item, qty_g: newQty } : item))
      );
    } catch (error) {
      console.error(error);
      ...
      {(mealRisk.redAlert || mealRisk.orangeAlert) && (
        <div
          style={{
            ...
            padding: 14,
            borderRadius: 12,
            border: mealRisk.redAlert ? "1px solid #f3b8b2" : "1px solid #f5d48a",
                  ...
                  <tr key={item.id}>
                    <td style={tdStyle}>
                      <a
                        ...
                        style={{ fontWeight: 600, color: "#1a73e8", textDecoration: "none" }}
                      >
                        {item.foods.name_ar}
                      </a>
                    ...
                    <td style={tdStyle}>
                      <input
                        ...
                        style={{ width: 90, padding: 8, border: "1px solid #ddd", borderRadius: 8 }}
                      />
                    </td>
